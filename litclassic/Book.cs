...
namespace litclassic
{
    class Book
    {
        ...
        private Line TitleIdentificationAndReturn(Tag tag, Line line)
        {
            ...
            // надо бы ещё добавить проверку самих "частиц" на наличие в них одновременно разных тем,
            // чтобы затем такие "частицы" исключать вовсе 
            ...
            if ((titleString != "\"\"") & (titleString != ""))
            {
                if (line.GetThemeType() != -1)
                {
                    if 
                        ...
                        || /*Жуковский, 1 т.*/ (titleString.ToLower() == "\"к тургеневу, в ответ на стихи, присланные им вместо письма\"")
                        || /*Жуковский, 3 т.*/ (titleString.ToLower() == "\"отрывки из «илиады»\"")

                        || /*Пушкин, 1 т.*/ (titleString.ToLower() == "\"из письма к кн. п.а. вяземскому\"")
                        || /*Пушкин, 1 т.*/ (titleString.ToLower() == "\"из письма к в.л. пушкину\"")
                        ...
                        || /*Толстой, 1 т.*/ (titleString.ToLower() == "\"андре шенье\"")
                        || /*Толстой, 1 т.*/ (titleString.ToLower() == "\"иоганн вольфганг гете\"")
                        || /*Толстой, 1 т.*/ (titleString.ToLower() == "\"генрих гейне\"")
                        || /*Толстой, 1 т.*/ (titleString.ToLower() == "\"шотландская народная поэзия\"")

                        // мне кажется, ради двух четверостиший не стоит
                        //|| /*Толстой, 1 т.*/ (titleString.ToLower() == "\"из письма\"")

                        || /*Тютчев, 2 т.*/ (titleString.ToLower().IndexOf("теперь не то, что за полгода") != -1)
                        || /*Тютчев, 2 т.*/ (titleString.ToLower().IndexOf("есть телеграф за неименьем ног") != -1))
                    {
                    ...
                    // от разделов в примечании с этим же названием
                        ...
                        /*Пушкин, 1 т.*/ (titleString.ToLower() == "\"воспоминания в царском селе\"")
                    {
                        if (line.GetThemeType() == 0)
                            line.SetThemeType(0);
                        ...
                        || /*Тютчев, 4 т.*/ (titleString.ToLower() == "\"вяземскому п. а., 11/23 июня 1837\"")
                        || /*Тютчев, 4 т.*/ (titleString.ToLower() == "\"вяземскому п. а., ноябрь — декабрь 1844\"")
                        || /*Тютчев, 4 т.*/ (titleString.ToLower() == "\"вяземскому п. а., 28 июля/9 августа 1847\"")
                        || /*Тютчев, 4 т.*/ (titleString.ToLower() == "\"вяземскому п. а., февраль 1848\"")
                        || /*Тютчев, 4 т.*/ (titleString.ToLower() == "\"вяземскому п. а., март 1848\""))                                                
                    {
                        // сделано для того, чтобы отделить материал с таким названием 
                        // от разделов в примечании с этим же названием
                        if (line.GetThemeType() != 3)
                        {
                            if ((titleString.ToLower().IndexOf("из письма к вигелю") == -1)
                                && (titleString.ToLower() != "\"из письма к вяземскому\"")
                                && (titleString.ToLower() != "\"вяземскому\""))
                                line.SetThemeType(1);
                            else line.SetThemeType(99);
                        }
                    }
                    else if // осн.разделы
                        ...
                        || /*Тютчев, п.с.стихотв.*/ (titleString.ToLower().IndexOf("ф. и. тютчев") != -1)
                        ...
                        || (titleString.ToLower().IndexOf("словарь мифологических имен") != -1))                 
                    {
                        ...
                        || /*Пушкин, 1 т.*/ (titleString.ToLower().IndexOf("перечень иллюстраций") != -1)
                        ...
                        || /*Пушкин, 1 т.*/ (titleString.ToLower() == "переводы иноязычных текстов")

                        || /*Пушкин, 1 т.*/ (titleString.ToLower().IndexOf("содержание") != -1)
                    ...
                    // Пушкинские исключения
                        ...
                        || /*1 том*/ (titleString.ToLower() == "\"юрьеву\"")
                        || /*1 том*/ (titleString.ToLower() == "\"на аракчеева\"")
                        || /*1 том*/ (titleString.ToLower() == "\"на кн. а.н. голицына\"")
                        ...
                        || /*2 том*/ (titleString.ToLower() == "\"дельвигу\"")
                        || /*2 том*/ (titleString.ToLower().IndexOf("на а. а. давыдову") != -1)
                        || /*2 том*/ (titleString.ToLower() == "\"христос воскрес\"")
                        || /*2 том*/ (titleString.ToLower() == "\"десятая заповедь\"")                     
                        || /*2 том*/ (titleString.ToLower().IndexOf("раззевавшись от обедни") != -1)
                        || /*2 том*/ (titleString.ToLower().IndexOf("вяземскому") != -1)
                        || /*2 том*/ (titleString.ToLower().IndexOf("на а. а. давыдову") != -1)
                        || /*2 том*/ (titleString.ToLower().IndexOf("мой друг, уже три дня") != -1)
                        || /*2 том*/ (titleString.ToLower().IndexOf("царь никита") != -1)
                        // исключить?
                        ...
                        || /*3 том*/ (titleString.ToLower().IndexOf("сводня грустно за столом") != -1)
                        || /*3 том*/ (titleString.ToLower() == "\"рефутация г-на беранжера\"")
                        || /*3 том*/ (titleString.ToLower() == "\"из альбома а. п. керн\"")
                        || /*3 том*/ (titleString.ToLower().IndexOf("на картинки к «евгению онегину» ") != -1)
                        || /*3 том*/ (titleString.ToLower() == "\"на дондукова-корсакова\"")
                        || /*3 том*/ (titleString.ToLower().IndexOf("к кастрату раз пришел скрыпач") != -1)
